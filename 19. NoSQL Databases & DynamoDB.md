...
2. Global Secondary Indexes (GSI)
	...
	\- strictly eventually consistent.
...
Conditional Expressions
	\- primarily used to determine which items should be modified for data manipulation operations such as PutItem, UpdateItem, and DeleteItem calls.

Project Expressions
	\- a string that identifies the attributes you want. 
	\- to retrieve a single attribute, specify its name.
	\- for multiple attributes, the names must be comma-separated.

Filter Expressions
	\- simply determines which items within the Query results should be returned to you.

Conditional Writes
	\- write operations will succeed only if the item attributes meet one or more expected conditions. Otherwise, it returns an error.

Atomic Counter
	\- a numeric attribute that is incremented, unconditionally, without interfering with other write requests.
	\- the numeric value will increment each time you call UpdateItem.

**Exceptions and Error Responses**

RequestLimitExceeded
	\- provisioned throughput exceeds the current throughput limit for your account.

Throttling Exception
	\- rate of requests exceeds the allowed throughput.

ProvisionedThroughputExceededException
	\- provisioned write capacity for the global secondary index is less than the write capacity of the base table.

DescribeTable
	\- provides detailed information about all of the table’s global secondary indexes will be returned.

ReturnConsumedCapacity
	\- return the number of write capacity units consumed by any of these operations (PutItem, UpdateItem, and DeleteItem).
...
* NONE (Default)
	\- no write capacity details are returned.

dynamodb:LeadingKeys
	\- allows users to access only the items where the partition key value matches their user ID.

dynamodb:Attributes
	\- limits access to the specified attributes so that only the actions listed in the permissions policy can return values for these attributes.

dynamodb:ReturnValues
	\- used for getting the item attributes as they appear before or after they are updated.

dynamodb:Select
	\- for specifying attributes to be returned in the result of a Query or Scan request.

BatchWriteItem
	\- does not support UpdateItem.

TransactionWriteItem
	\- a synchronous and idempotent write operation that groups up to 25 write actions in a single all-or-nothing operation.
