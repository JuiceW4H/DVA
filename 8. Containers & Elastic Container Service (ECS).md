...
4. cloud-controller-manager (Optional Component)
...
* Service
	...
	\- an application can be containerized across many pods, but the service is the consistent thing in the abstraction.
...
* If you terminate a container instance in the RUNNING state, that container instance is automatically removed or deregistered from the cluster. However, if you terminate a container instance in the STOPPED state, that container instance isn’t automatically removed from the cluster. (ECS)